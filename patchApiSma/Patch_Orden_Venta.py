...
PATCH_BASE_URL = "http://192.168.5.10:8081/SMA_WEBAPI_WMS/WS/Services/Orders/1/"
...
# Procesar cada DocEntry con nuevo token
for doc_entry in docentries:
    token = get_token()
    if token:
        patch_docentry(doc_entry, token)
    else:
        print(f"🚫 Token no obtenido. Se salta DocEntry {doc_entry}.")
