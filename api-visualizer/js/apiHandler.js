...
document.getElementById("export-json").addEventListener("click", function () {
    ...
    // Recorrer las filas de la tabla y extraer los datos correctamente
    resultsBody.querySelectorAll("tr").forEach(row => {
        let cell = row.querySelector("td pre"); // Buscar el <pre> dentro de la celda
        if (cell) {
            try {
                let jsonData = JSON.parse(cell.textContent); // Convertir el texto a JSON real
                data.push(jsonData);
            } catch (error) {
